...
```
2 pages look scanned (image only) and indexed empty: no vision model is running to read them.
To make scans searchable: hay llm setup installs one (llava), then re-add this folder.
```
