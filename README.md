...
Everything lives in a single SQLite database on your disk. Semantic search uses local embeddings via [Ollama](https://ollama.com) when available; keyword search works with zero dependencies. Answers (`hay ask`) are generated by your local Ollama model — Haypile itself ships no LLM and makes no network calls.
...
| v2 | Bundled embeddings (no Ollama dependency), ANN index for very large corpora |
