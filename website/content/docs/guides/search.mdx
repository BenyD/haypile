...
PDFs cite a page, and each slide in a pptx cites its slide as a page. Formats without fixed pages (Markdown, text, docx, HTML) cite a chunk position instead. The citation is the contract: if a result cannot tell you where it came from, it does not belong in the output.
