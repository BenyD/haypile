...
Embedding is also built to lose fights for the machine: it fans out across all CPU cores minus two of headroom, and the daemon runs itself at background priority. An idle machine indexes at full speed; a busy one yields to whatever you are actually doing, at the cost of a somewhat longer first index.
